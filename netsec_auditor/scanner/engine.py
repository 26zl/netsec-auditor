"""Core scanner engine — port scanning, service detection, and OS fingerprinting."""
...
def is_privileged() -> bool:
    """True if the process can run privileged nmap scans (root on POSIX)."""
    try:
        return os.geteuid() == 0
...
class _RateLimiter:
    """Caps how many operations may start per second (rate <= 0 means unlimited)."""
...
COMMON_PORTS = [
    ...
    1723, 3306, 3389, 5432, 5900, 6379, 8080, 8443, 9000, 9090, 9200, 27017,
]
...
TOP_1000_PORTS = [
    ...
    1112, 1113, 1114, 1117, 1119, 1121, 1122, 1123, 1124, 1126, 1130, 1131,
    1132, 1137, 1138, 1141, 1145, 1147, 1148, 1149, 1151, 1152, 1154, 1163,
    1164, 1165, 1166, 1169, 1174, 1175, 1183, 1185, 1186, 1187, 1192, 1198,
    1199, 1201, 1213, 1216, 1217, 1218, 1233, 1234, 1236, 1244, 1247, 1248,
    ...
    3011, 3013, 3017, 3030, 3031, 3050, 3052, 3071, 3077, 3128, 3168, 3211,
    ...
    7200, 7402, 7435, 7443, 7496, 7512, 7625, 7627, 7676, 7741, 7777, 7778,
    ...
    9593, 9594, 9595, 9618, 9666, 9876, 9877, 9878, 9898, 9988, 9999, 10000,
...
class PortState(Enum):
    ...
    @classmethod
    def parse(cls, value: str) -> PortState:
        """Map an nmap state string to a PortState, defaulting to UNKNOWN."""
        try:
            return cls(value)
        except ValueError:
            return cls.UNKNOWN
...
class ServiceInfo:
    ...
    extrainfo: str = ""
    ostype: str = ""
    tunnel: ServiceTunnel = ServiceTunnel.NONE
...
class PortScanner:
    """Asynchronous port scanner using python-nmap with scope enforcement."""
    ...
    async def scan_host(
        ...
        if self.scope.allowed_ports is not None:
            ...
            if len(allowed) != len(ports):
                logger.warning(
                    "ports_filtered_by_scope",
                    host=target, requested=len(ports), allowed=len(allowed),
                )
            ports = allowed
            if not ports:
    ...
    async def scan_network(
        ...
        if (scan_type.lower() in ("syn", "udp") or os_detection) and not is_privileged():
            logger.warning(
                ...
                scan_type=scan_type,
                hint="run with sudo or use --scan-type connect",
            )
    ...
    def _parse_nmap_result(self, target: str, duration: float) -> HostResult:
        ...
        if "vendor" in host:
            vendor = list(host["vendor"].values())[0] if host["vendor"] else ""
        ...
        if "osmatch" in host and host["osmatch"]:
            ...
            os = best_match.get("name", "")
            os_accuracy = int(best_match.get("accuracy", 0))
            if "osclass" in best_match and best_match["osclass"]:
                os_family = best_match["osclass"][0].get("osfamily", "")

        ports: list[PortResult] = []
        for proto in ("tcp", "udp"):
            if proto in host:
                for port_num in host[proto]:
                    port_data = host[proto][port_num]
                    state = PortState.parse(port_data.get("state", "unknown"))
                    service = None

                    if state == PortState.OPEN and "name" in port_data:
                        service = ServiceInfo(
...
class BannerGrabber:
    """TCP banner grabbing for service fingerprinting."""
    ...
    async def grab_banner(self, host: str, port: int) -> str:
        ...
        try:
            ...
            logger.debug("banner_grab_failed", host=host, port=port, error=str(e))
            return ""

    async def grab_banners(
        self, host: str, ports: list[int], concurrency: int = 20
    ) -> dict[int, str]:
        """Grab banners from multiple ports concurrently."""
        semaphore = asyncio.Semaphore(concurrency)

        async def _grab_one(port: int) -> tuple[int, str]:
            async with semaphore:
                banner = await self.grab_banner(host, port)
                return port, banner

        tasks = [_grab_one(p) for p in ports]
        results = await asyncio.gather(*tasks)
        return dict(results)


class NetworkDiscovery:
    """Network discovery — ping sweeps and ARP scans."""
    ...
    async def ping_sweep(self, cidr: str) -> list[str]:
        """Discover live hosts via unprivileged TCP probes to common ports.

        A host is alive if any probe connects or is refused (both prove
        reachability). Hosts that silently drop packets are missed — use a
    ...
    async def arp_scan(self, cidr: str) -> list[dict[str, str]]:
        """Discover live hosts via ARP scan (requires scapy, root)."""
        self.scope.validate_target(cidr)

        try:
            from scapy.all import ARP, Ether, srp
        except ImportError:
            logger.warning("scapy_not_installed")
            return []
        ...
        answered, _ = await loop.run_in_executor(
            None, lambda: srp(packet, timeout=3, verbose=0)
        )
        ...
        logger.info("arp_scan_complete", cidr=cidr, hosts=len(results))
        return results
