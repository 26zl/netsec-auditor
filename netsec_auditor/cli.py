...
def web(
    urls: Annotated[
        list[str],
        typer.Argument(help="Device/host URLs to audit (admin panels, embedded web servers)"),
    ],
    scope_file: Annotated[
        Path | None,
        typer.Option("--scope", "-s", exists=True, help="Scope definition YAML"),
    ...
    """Audit a host's web interface — device admin panels, embedded servers — for posture."""
    scope = _load_scope(scope_file, urls)
    if concurrency < 1:
        raise typer.BadParameter("concurrency must be at least one")

    console.print(Panel.fit(
        f"[bold magenta]NetSec Auditor — Web Interface Audit[/bold magenta]\n"
    ...
    async def _run() -> None:
        ...
        if output and results:
            ...
            paths = report_gen.generate_all(
                web_results=results,
                title="Web Interface Security Report",
                scope_name=scope.name,
            )
            console.print("\n[green]Reports generated:[/green]")
            for fmt_name, path in paths.items():
...
def full(
    ...
    deep_web: Annotated[
        bool,
        typer.Option("--deep-web", help="Deep web-interface scan (directory enumeration)"),
    ] = False,
    cve_check: Annotated[
        bool,
        typer.Option("--cve-check/--no-cve-check", help="Query NVD for CVEs"),
    ...
    async def _run() -> None:
        ...
        web_results = []
        if web_scanner:
            console.print("\n[bold]Phase 3/4: Web Interface Audit[/bold]")
